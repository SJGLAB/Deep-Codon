...
def read_test(input_file, word_alphabet, char_alphabet, feature_alphabets, label_alphabet, number_normalized, max_sent_length, char_padding_size=-1, char_padding_symbol = '</pad>'):
    feature_num = len(feature_alphabets)
    in_lines = open(input_file,'r').readlines()
    instence_texts = []
    instence_Ids = []
...
def recover_label(pred_variable, gold_variable, mask_variable, label_alphabet, word_recover):
    ...
    batch_size,seq_len = gold_variable.size(0),gold_variable.size(1)
    mask = mask_variable.cpu().data.numpy()
...
def recover_nbest_label(pred_variable, mask_variable, label_alphabet, word_recover):
    ...
    pred_variable = pred_variable[word_recover]
    mask_variable = mask_variable[word_recover]
    batch_size,seq_len,nbest = pred_variable.size(0),pred_variable.size(1),pred_variable.size(2)
